...
def statMarket(conn,log):
	nullsKeys = ('unbalanced_qualities','corrupted_draft','corrupted_stock_s','corrupted_stock_a')
	...
	with db.Cursor(conn) as cursor:
		...
		while True:
			...
			for k in nullsKeys:
				if res[k]!=0:
					exitError(log,'ob_fstats().%s = %i'%(k,res[k]))
...
def getLastBid(conn,log):
	...
	with db.Cursor(conn) as cursor:
		...
		res = cursor.fetchone()
		version = res[0]
		print "nid=%i version=%i" %(nbids,version)
