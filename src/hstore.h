...
#define HSCALCDATASIZE(x, lenstr) ( (x) * 2 * sizeof(HEntry) + HSHRDSIZE + (lenstr) )

/* note multiple evaluations of x */
#define HSARRPTR(x)		( (HEntry*) ( (HStore*)(x) + 1 ) )
#define HSSTRPTR(x)		( (char*)(HSARRPTR(x) + HS_COUNT((HStore*)(x)) * 2) )
...
#define HS_FINALIZE(hsp_,count_,buf_,ptr_)							\
	do {															\
		...
		if ((count_))												\
			HSARRPTR(hsp_)[0].entry |= HENTRY_ISFIRST;				\
		if ((count_) != HS_COUNT((hsp_)))							\
		{															\
			HS_SETCOUNT((hsp_),(count_));							\
			memmove(HSSTRPTR(hsp_), (buf_), buflen);					\
		}															\
		SET_VARSIZE((hsp_), HSCALCDATASIZE((count_), buflen));		\
	} while (0)

/* ensure the varlena size of an existing hstore is correct */
#define HS_FIXSIZE(hsp_,count_)											\
	do {																\
		int bl = (count_) ? HSE_ENDPOS(HSARRPTR(hsp_)[2*(count_)-1]) : 0; \
		SET_VARSIZE((hsp_), HSCALCDATASIZE((count_),bl));					\
	} while (0)
